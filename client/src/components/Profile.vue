<template>
                ...
                <div class="about__me">
                    <form action="">
                        <p class="about__me__txt" > О себе</p>>
                            <p><textarea name="comment" cols="40" rows="3" v-model="description"></textarea></p>
        ...
        <div class="container">
            ...
            <div class="anketa__block">
                <div class="anketa_label">
                    <label class="anketa_specialist">Специальность<sup>*</sup>  </label>
                    <select id="selectvalue" class="anketa_specialis_sel" v-model="value_speciality" @click="output_console_value(value_speciality)">
                ...
                <div class="anketa_label">
                    ...
                    <select id="selectvalue" class="anketa_stack_sel" v-model="value_stack" @click="output_console_value(value_stack)">
                        <option v-if="value_speciality === dict_speciality.front.ru" v-for="(item, index) in dict_stack.front">
                            {{ item }}
                        </option>
                        <option v-else-if="value_speciality === dict_speciality.back.ru" v-for="(item, index) in dict_stack.back">
                            {{ item }}
                        </option>
                        <option v-else-if="value_speciality === dict_speciality.design.ru" v-for="(item, index) in dict_stack.design">
                            {{ item }}
                        </option>
                        <option v-else-if="value_speciality === dict_speciality.layout.ru" v-for="(item, index) in dict_stack.layout">
                            {{ item }}
                        </option>
                    </select>
                </div>
                <div class="anketa_label">
                    <label class="anketa_pract">Опыт<sup>*</sup>  </label><select id="selectvalue" class="anketa_pract_sel" v-model="value_experience" @click="output_console_value(value_experience)">
                        <option v-for="(item, index) in dict_experience">
                            {{ item }}
                        </option>
                </select>
                </div>
                <p class="about__me__btn"><input type="submit" value="Сохранить"></p>
            </div>
    ...
    <section class="connect_profile">
        <div class="container">
            ...
            <p class="connect__desc">Для связи с будущими тиммейтами необходимо указать контакты, по которым можно будет с вами связаться.</p>
            <div class="connect__block">
                <label>Telegram<sup>*</sup>  <input type="text" v-model="telegram"/></label>
                <label class="discord_profile">Discord     <sup>*</sup>  <input  type="text" v-model="discord"/></label>
                <p class="about__me__btn"><input @click="post" value="Сохранить"></p>
            </div>
        </div>
    </section>
</template>
